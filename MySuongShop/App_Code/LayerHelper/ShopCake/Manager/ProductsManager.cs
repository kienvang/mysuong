...
namespace LayerHelper.ShopCake.BLL
{
    public class ProductsManager : ProductsManagerBase
    {
        ...
        public DataTable Search(string ProductName, string Price, string catalog)
        {
            ...
            strSQL += tmp + " Order By p.CreatedDate Desc";
        ...
        public string Resize(int height, int width, ProductsEntity product)
        {
            if (string.IsNullOrEmpty(product.Thumbnail)) return "/images/no_img.gif";
            string path = HttpContext.Current.Server.MapPath(product.Thumbnail);

            string str_resize = Path.GetDirectoryName(product.Thumbnail) + "/" + Path.GetFileNameWithoutExtension(product.Thumbnail) + "_fb_200" + Path.GetExtension(product.Thumbnail);

            string path_resize = Path.GetDirectoryName(path) + "/" + Path.GetFileNameWithoutExtension(path) + "_fb_200" + Path.GetExtension(path);
            if (!File.Exists(path_resize))
            {
                Image image = System.Drawing.Image.FromFile(path);
                int w = image.Width;
                int h = image.Height;
                ImageFunction.GetScaledWidthHeight(image.Width, image.Height, width, height, out w, out h);

                using (Image thumbnailImage = Library.Tools.ImageFunction.ScaleImage(image, w, h, width, height))
                {
                    thumbnailImage.Save(path_resize);
                }
            }
            return str_resize.Replace("\\", "/");
        }
    }
}
